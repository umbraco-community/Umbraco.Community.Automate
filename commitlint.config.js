...
// Dynamically discover and load all scopes from package config files
function loadScopes() {
    const scopes = new Set();
    const rootDir = __dirname;

    // Find all Umbraco.Community.Automate.* package directories
    const entries = fs.readdirSync(rootDir, { withFileTypes: true });

    for (const entry of entries) {
        if (!entry.isDirectory()) continue;
        if (!entry.name.startsWith("Umbraco.Community.Automate.")) continue;

        const configPath = path.join(rootDir, entry.name, "changelog.config.json");

        // Load package scopes from changelog.config.json (only present once a
        // package opts into the changelog tooling — see CLAUDE.md's "Deferred
        // Tooling" section).
